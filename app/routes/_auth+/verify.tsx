...
async function validateRequest(
	...
	async function deleteVerification() {
		await storageContext.db.verification.delete({
			where: {
				target_type: {
	...
	switch (submissionValue[typeQueryParam]) {
		case 'reset-password': {
			...
			return handleResetPasswordVerification({ storageContext, request, body, submission })
		}
		case 'onboarding': {
			await deleteVerification()
			return handleOnboardingVerification({ storageContext, request, body, submission })
		}
		case 'change-email': {
			await deleteVerification()
			return handleChangeEmailVerification({ storageContext, RESEND_API_KEY, MOCKS, request, body, submission })
		}
		case '2fa': {
			return handleLoginTwoFactorVerification({ storageContext, request, body, submission })
		}
	}
}
