...
import { isCodeValid, twoFAVerificationType, twoFAVerifyVerificationType } from '#app/utils/verification.server.ts'
...
export async function action({ context: { storageContext }, request }: ActionFunctionArgs) {
	...
	const submission = await parseWithZod(formData, {
		schema: () =>
			ActionSchema.superRefine(async (data, ctx) => {
				...
				const validCode = await isCodeValid({
					storageContext,
					code: data.code,
	...
	switch (submission.value.intent) {
		...
		case 'verify': {
			...
			return redirectWithToast(toastSessionStorage, '/settings/profile/two-factor', {
				type: 'success',
				title: 'Enabled',
	...
	return (
		<div>
			<div className="flex flex-col items-center gap-4">
				...
				<p className="text-sm">
					Once you&apos;ve added the account, enter the code from your authenticator app below. Once you enable 2FA, you
					will need to enter a code from your authenticator app every time you log in or perform important actions. Do
					not lose access to your authenticator app, or you will lose access to your account.
				</p>
