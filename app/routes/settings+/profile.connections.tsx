...
import { StorageContext, requireUserId } from '#app/utils/auth.server.ts'
...
async function userCanDeleteConnections(storageContext: StorageContext, userId: string) {
...
export async function loader({ context: { storageContext }, request }: LoaderFunctionArgs) {
	...
	const rawConnections = await storageContext.db.connection.findMany({
	...
	for (const connection of rawConnections) {
		...
		const connectionData = await resolveConnectionData(providerName, connection.providerId, { timings })
		connections.push({
			...connectionData,
			providerName,
			id: connection.id,
...
export async function action({ context: { storageContext }, request }: ActionFunctionArgs) {
	...
	invariantResponse(
		await userCanDeleteConnections(storageContext, userId),
		'You cannot delete your last connection unless you have a password.',
	)
	const connectionId = formData.get('connectionId')
	invariantResponse(typeof connectionId === 'string', 'Invalid connectionId')
	await storageContext.db.connection.delete({
		where: {
			id: connectionId,
			userId: userId,
		},
	})
	const toastHeaders = await createToastHeaders(storageContext.toastSessionStorage, {
		title: 'Deleted',
			...
			{canDelete ? (
				...
				<TooltipProvider>
					<Tooltip open={infoOpen} onOpenChange={setInfoOpen}>
						...
						<TooltipContent>You cannot delete your last connection unless you have a password.</TooltipContent>
					</Tooltip>
				</TooltipProvider>
			)}
		</div>
